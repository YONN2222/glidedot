...
const isReviewEnabled = ref(false)
...
onMounted(async () => {
  ...
  try {
    ...
    if (project) {
      ...
      isReviewEnabled.value = project.reviewEnabled || false
    }
  } catch (e) {
...
const toggleReviewMode = async () => {
  if (!currentProject.value || isUpdatingSettings.value) return
  isUpdatingSettings.value = true

  try {
    const newValue = !isReviewEnabled.value
    await fetchApi(`/localization/projects/${currentProject.value.id}`, {
      method: 'PATCH',
      body: { reviewEnabled: newValue }
    })
    
    isReviewEnabled.value = newValue
    
    // Update local state
    if (currentProject.value) {
      currentProject.value.reviewEnabled = newValue
    }

    toast.add({ title: 'Success', description: `Global Review mode ${newValue ? 'enabled' : 'disabled'}.`, color: 'success' })
  } catch (error) {
    toast.add({ title: 'Error', description: (error as Error).message || 'Failed to update global review mode', color: 'error' })
  } finally {
...
<template>
  <div class="flex flex-col gap-6 -m-4 sm:-m-6 lg:-m-8 p-4 sm:p-6 lg:p-8">
    <div class="flex flex-col md:flex-row justify-between md:items-center mb-2 gap-4">
      ...
      <div v-if="isAdmin" class="hidden md:flex items-center justify-end gap-3 bg-neutral-900/50 px-4 py-2 rounded-xl border border-neutral-800">
        <span class="text-sm font-medium" :class="isReviewEnabled ? 'text-primary' : 'text-neutral-400'">
          Global Review Mode
        </span>
        <u-switch 
          :model-value="isReviewEnabled" 
          :loading="isUpdatingSettings" 
          @update:model-value="toggleReviewMode"
        />
      </div>
    </div>
